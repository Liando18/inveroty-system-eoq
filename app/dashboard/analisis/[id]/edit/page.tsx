...
export default function EditAnalisisPage({
  ...
  const [periode, setPeriode] = useState<number>(new Date().getFullYear());
  ...
  useEffect(() => {
    async function init() {
      ...
      if (resA.success && resA.data) {
        const d = resA.data;
        setProdukId(d.produk_id);
        setPeriode(d.periode);
        setBiayaPemesanan(d.biaya_pemesanan);
        setBiayaPenyimpanan(d.biaya_penyimpanan);
        setLamaPemesanan(d.lama_pemesanan);
        setPermintaanTahun(d.permintaan_pertahun);
        setSafetyStock(d.stok_cadangan);
      } else {
        setErrorMsg("Data analisis tidak ditemukan.");
      }
      setLoading(false);
    }
    init();
  ...
  useEffect(() => {
    if (loading || !produkId || !periode) return;
    async function run() {
      setIsFetchingAuto(true);
      const [demand, ss] = await Promise.all([
        getPermintaanTahun(produkId, periode),
  ...
  async function handleSave() {
    setErrorMsg("");
    if (!produkId || !periode) return setErrorMsg("Pilih produk dan periode.");
    if (biayaPemesanan <= 0 || biayaPenyimpanan <= 0)
      return setErrorMsg("Biaya pemesanan & penyimpanan harus lebih dari 0.");
    setSaving(true);
    const res = await updateAnalisis(paramId, {
      produk_id: produkId,
      periode,
  ...
  const inputClass =
    "w-full px-4 py-3 rounded-xl border border-gray-200 text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 focus:ring-amber-200 focus:border-amber-400 transition-all";
  const labelClass =
  ...
  return (
    <div style={{ paddingBottom: "48px" }}>
      <div
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Edit Analisis</h1>
          <p className="text-sm text-gray-400 mt-1">
            Ubah parameter perhitungan EOQ &amp; ROP
          </p>
        </div>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div style={{ padding: "24px" }}>
              ...
              <div>
                <label className={labelClass}>Lead Time / L (hari)</label>
                <input
                  type="number"
                  value={lamaPemesanan || ""}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                  ...
                  {[
                    ...
                    <div
                      ...
                      <span
                        ...
                        {row.text}
                        <b style={{ color: "#374151" }}>{row.bold}</b>
                      </span>
                    </div>
                  ))}
                  <div
          ...
          <div
            ...
            <button
              ...
              style={{
                padding: "16px",
                background: saving || !hasResult ? "#f3f4f6" : "#f59e0b",
