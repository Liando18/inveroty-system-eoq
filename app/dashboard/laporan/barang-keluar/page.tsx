...
import {
  getLaporanBarangKeluarHarian,
  getLaporanBarangKeluarBulanan,
  getLaporanBarangKeluarTahunan,
  type LaporanBarangKeluar,
  type LaporanBarangKeluarAggregated,
...
export default function LaporanBarangKeluarPage() {
  ...
  const [dataHarian, setDataHarian] = useState<LaporanBarangKeluar[]>([]);
  const [dataAggregated, setDataAggregated] = useState<
    LaporanBarangKeluarAggregated[]
  >([]);
  ...
  async function fetchData() {
    ...
    if (activeTab === "harian") {
      const result = await getLaporanBarangKeluarHarian(tanggal);
      if (result.success) {
        setDataHarian(result.data);
      }
    } else if (activeTab === "bulanan") {
      const result = await getLaporanBarangKeluarBulanan(tahun, bulan);
      if (result.success) {
        setDataAggregated(result.data);
      }
    } else {
      const result = await getLaporanBarangKeluarTahunan(tahun);
      if (result.success) {
        setDataAggregated(result.data);
      }
    }
  ...
  const totalBarang =
    activeTab === "harian"
      ? dataHarian.reduce(
          (sum, item) =>
            sum + item.details.reduce((s, d) => s + d.jumlah_jual, 0),
          0,
        )
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div className="flex flex-wrap items-end gap-4">
          {activeTab === "harian" && (
            <div>
              ...
              <input
                ...
                className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              />
            </div>
          )}
            ...
            <>
              <div>
                ...
                <select
                  ...
                  className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent min-w-[140px]">
                  {bulanOptions.map((b) => (
              ...
              <div>
                ...
                <input
                  ...
                  className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent w-28"
                />
              </div>
            </>
          )}
          ...
          <button
            onClick={fetchData}
            className="px-4 py-2.5 rounded-xl bg-orange-500 text-white text-sm font-semibold hover:bg-orange-600 transition">
            Tampilkan
          </button>
        </div>
      </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {["#", "Kode", "Tanggal", "Detail", "Total", "PIC"].map(
                      (h) => (
                        <th
                          key={h}
                  ...
                  {dataHarian.map((m, i) => (
                    <tr
                      ...
                      <td className="px-4 py-4 text-center">
                        <span className="bg-orange-50 text-orange-700 px-2.5 py-1 rounded-md text-xs font-bold tracking-widest uppercase">
                          {m.kode}
                        </span>
                      </td>
                          ...
                          {m.details.map((d, idx) => (
                            <div
                              ...
                              <span className="text-gray-800 font-medium">
                                {d.jumlah_jual} x Rp{" "}
                                {d.harga_jual.toLocaleString("id-ID")}
                              </span>
                            </div>
                          ))}
                        </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {dataAggregated.map((item, i) => (
                  <tr key={i} className="hover:bg-gray-50/60 transition-colors">
                    ...
                    <td className="px-5 py-4 text-sm font-bold text-orange-700 text-center">
