...
export default function EditBarangKeluarPage({
  ...
  useEffect(() => {
    async function init() {
      ...
      const [resP, resKeluar, resStok] = await Promise.all([
        getAllProducts(),
        getBarangKeluar(paramId),
        getAllStok(),
      ]);
      ...
      if (resKeluar.success && resKeluar.data) {
        setKode(resKeluar.data.kode);
        setTanggal(
          new Date(resKeluar.data.tanggal).toISOString().split("T")[0],
        );
        const mappedDet = resKeluar.data.barang_keluar_detail.map((d: any) => ({
        ...
        setErrorMsg("Data barang keluar tidak ditemukan.");
      }
      setLoading(false);
    }
    init();
  ...
  async function handleSave() {
    ...
    const res = await updateBarangKeluarTransaction(
      paramId,
      payloadKeluar,
      payloadDetails,
    );
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Edit dan hitung ulang stok keluar. Kode: {kode}
          </p>
        </div>
      </div>
