...
import { createBarangKeluarTransaction } from "@/app/controller/barang-keluar.controller";
...
export default function TambahBarangKeluarPage() {
  ...
  const [items, setItems] = useState<
    { produk_id: number; harga_jual: number; jumlah_jual: number }[]
  >([]);
  ...
  useEffect(() => {
    async function init() {
      const [resP, resStok] = await Promise.all([
        getAllProducts(),
        getAllStok(),
      ]);
  ...
  function handleAddItem() {
    ...
    setItems([
      ...items,
      {
        produk_id: firstProduct.id,
        harga_jual: firstProduct.harga,
        jumlah_jual: 1,
      },
    ]);
  }
  ...
  const grandTotal = items.reduce(
    (acc, curr) => acc + curr.harga_jual * curr.jumlah_jual,
    0,
  );
  ...
  async function handleSave() {
    ...
    if (items.some((x) => x.harga_jual <= 0 || x.jumlah_jual <= 0))
      return setErrorMsg("Harga dan jumlah harus lebih dari 0.");
    ...
    const payloadDetails = items.map((x) => ({
      ...
      jumlah_jual: x.jumlah_jual,
    }));

    const res = await createBarangKeluarTransaction(
      payloadKeluar,
      payloadDetails,
    );
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Catat penjualan / barang keluar. Otomatis kurangi stok (-)
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-400 font-medium"
            />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-2">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-50 text-red-700 text-xs font-bold hover:bg-red-100 transition">
              <svg
                className="w-4 h-4"
                fill="none"
                stroke="currentColor"
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <div className="w-full sm:w-1/2">
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-400 bg-white shadow-sm">
                        {products.map((p) => (
                      ...
                      <div className="w-full sm:w-1/2 bg-white px-4 py-3 rounded-lg border border-gray-100 shadow-sm flex flex-col justify-center">
                        ...
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-semibold text-gray-600">
                            Stok:{" "}
                            <span
                              className={`font-bold ${currentStock <= 0 ? "text-red-500" : "text-gray-900"}`}>
                              {currentStock}
                            </span>
                          </span>
                        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end">
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-400 shadow-sm"
                        placeholder="1"
                      />
                    </div>
                    <div className="sm:col-span-4">
                      ...
                      <div className="px-3 py-2.5 bg-white border border-gray-200 rounded-lg text-sm font-semibold text-gray-700 shadow-sm h-[42px] flex items-center">
                        Rp{" "}
                        {(item.harga_jual * item.jumlah_jual).toLocaleString(
                          "id-ID",
                        )}
                      </div>
                    </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="w-full sm:w-auto bg-red-50 px-5 py-3 rounded-xl border border-red-100 text-center sm:text-left">
